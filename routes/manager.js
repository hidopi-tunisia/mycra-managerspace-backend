...
router.put("/update-manager/:id", async (req, res) => {
    try {
      ...
      const updatedManager = await Manager.findByIdAndUpdate(managerId, updatedData, {
      ...
      res.json({ message: "Manager mis à jour avec succès", manager: updatedManager });
    } catch (error) {
      console.log(error);
      res.status(500).json({ message: "Erreur lors de la mise à jour du manager" });
    }
  });
...
router.get("/manager/:id", async (req, res) => {
    try {
      ...
      const manager = await Manager.findById(managerId).populate("consultants").exec();
      ...
      console.log(error);
      res.status(500).json({ message: "Erreur lors de la récupération des informations du manager" });
    }
  });

// Endpoint pour afficher le nombre de jours travaillés des consultants d'un manager
router.get("/consultants/:managerId/nombre-jours-travailles", async (req, res) => {
    try {
